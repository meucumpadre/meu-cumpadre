...
Versao 3.0  |  Abril 2026  |  Integracao Systems Claude/Meu Cumpadre

Autor: Alessandro de Souza Neves (SIAPE 1.378.671)  |  Co-autor: Claude Opus 4.7 (Anthropic)
Revisao v3.0 (20/04/2026): correcao Router-Ethics v9->v10 (blocos D/C/V, 3 scores independentes), WhatsApp Cloud API consolidada (migracao concluida), ADR-009a/b incorporados, Operacao Crisalida Z3 refletida, Evolution API removida do futuro.
Supersede: v2.0 (marco/2026)
...
| router\_ethics\_score | Calcula 3 scores independentes: confidence (C\_conf), complexity (C\_comp), fraudscore (C\_fraud). Retorna decisao AUTONOMIA vs HANDOFF com XAI. | Log auditavel | Nao (automatico) |
...
## **4.1  WhatsApp Cloud API — Migracao Concluida (ADR-008, 15/04/2026)**

*[ATUALIZADO v3.0 — migracao concluida, nao mais pendente]*

A migracao da Evolution API (open-source, Baileys) para WhatsApp Cloud API (oficial Meta) foi concluida e documentada no ADR-008 (selado 15/04/2026). A Evolution API depende de engenharia reversa do protocolo WhatsApp, risco operacional inaceitavel para atendimento a hipervulneraveis. A decisao original esta no registro D01-D08.

O MCP Server custom para WhatsApp Cloud API esta implementado sobre Streamable HTTP com suporte a: envio e recebimento de mensagens texto, audio transcrito via Whisper, imagens processadas via OCR, templates pre-aprovados pela Meta, e webhooks para notificacoes de status. Evolution API = descartada definitivamente.

*Referencia: MC-ADR-008-DecisoesFerramenta_Stack-v1_0-2026-0415.md*
...
| Orquestrador Etico | Claude Sonnet 4.6 (via MCP) | Casos complexos, Router-Ethics Confidence <70 ou Complexity alta, XAI | Apenas contexto anonimizado | Alto (~5% dos casos) |
...
*[CORRIGIDO v3.0 — terminologia v9.0 (blocos R/P/A, formula Score unico ≥39) substituida por v10.0 canonica (blocos D/C/V, 3 scores independentes, threshold Confidence <70)]*

O Router-Ethics 11.0 opera como Tool MCP nativo: recebe parametros estruturados (tipo_demanda, complexidade, vulnerabilidade, urgencia), executa arvore deterministica de 105 nos distribuidos em tres blocos hierarquicos D->C->V. Cada bloco so e avaliado se o anterior permitir — espelhando a hierarquia Dignidade > Compliance > Viabilidade.

| Bloco | Nos | Foco | Regra de Avaliacao |
| ----- | ----- | ----- | ----- |
| D — Dignidade | 35 nos (D01-D35) | Risco de vida, violencia, fome, idade >80 + analfabetismo, 2+ indeferimentos, 14 nos rurais/emocionais | PRIMEIRO — qualquer disparo = handoff imediato, demais blocos irrelevantes |
| C — Compliance | 35 nos (C36-C70) | OAB (atividade-meio), LGPD (consentimento, minimizacao), CFM, CNJ | Segundo — tolerancia zero a erros juridicos; 60% regras fixas, 40% probabilisticas |
| V — Viabilidade | 35 nos (V71-V105) | Complexidade documental, multiplos orgaos, litigio, custo operacional | Terceiro — unica camada onde otimizacao de custo pode entrar |

A v10.0 retorna TRES scores independentes (nao formula unica como na v9.0):

| Score | O Que Mede | Threshold Critico |
| ----- | ----- | ----- |
| Confidence (C_conf) | Certeza juridica da recomendacao | >= 70 VERDE (pipeline liberado); 40-70 AMARELA (triagem Anjo); < 40 VERMELHA (bloqueado) |
| Complexity (C_comp) | Friccao documental e processual | Alto -> Rota C obrigatoria (Tier 4 Advogado) |
| Fraudscore (C_fraud) | Risco de predacao contra o usuario | Alto -> bloqueio pipeline + notificacao antifraude |

HANDOFF obrigatorio se QUALQUER dos tres scores cruza seu threshold. AUTONOMIA somente se todos passam simultaneamente.

O Circuit Breaker Fenix e o mecanismo de seguranca final: se qualquer violacao grave de dignidade for detectada (D01 Desespero/Risco de vida, D06 Fome/Inseguranca alimentar, D13 Crise de saude mental), o sistema aborta automacao instantaneamente e aciona handoff urgente com log permanente no Tier 4.

*Referencia canonica: RouterEthics_10_MeuCumpadre_Unificado_v2__2026.md*
...
Score calculado em menos de 800ms. Se Confidence >= 70 e Complexity baixa e Fraudscore baixo e nenhum trigger D/C ativado: AUTONOMIA (fluxo automatizado continua). Se Confidence < 70, ou Complexity alta, ou Fraudscore alto, ou qualquer trigger D/C: HANDOFF (interrupt() pausa fluxo, Task Queue distribui para Anjo/estagiario/advogado conforme complexidade). Explicacao XAI gerada automaticamente em linguagem de 5a serie via Prompt MCP /explicacao\_xai.
...
**Custodia de senhas gov.br — ADR-009a (SELADO 17/04/2026):** Senha gov.br do usuario NUNCA armazenada no ClickUp (zero — nenhum campo, comentario, anexo). Bitwarden MC Organizations: AES-256, 2FA obrigatorio, audit log mensal. Modelo B Bitwarden. Protocolo de encerramento: orienta usuario a trocar senha + remove valor do Bitwarden, preserva metadata para auditoria.

**Tres Estados de Custodia do Dossie — ADR-009b (SELADO 17/04/2026):** MC atua como Fiduciario Digital, nao broker de comunicacao. S1 Intermediario (cidadao, MC custodia cifrado, 30-60d) -> S2 Consolidado (cidadao, MC entrega + apaga em 7d) -> S3 Probatorio (escritorio parceiro, controlador, 10 anos CED). Transicao S2->S3 requer Token de Transferencia Assinado (TTA) pelo cidadao. Zero transferencia sem consentimento explicito. *Referencia: MC-ADR-009-TresEstados-Custodia-Dossie-v1_0-2026-0417.md*
...
Casos complexos (Confidence < 70 ou Complexity alta) sao encaminhados para advogados parceiros via Marketplace Etico. O Meu Cumpadre cobra B2B lead fee (Psi_1 = R$200-800 por dossie, ADR-007 v3.0), nao honorarios advocaticios, com segregacao de notas fiscais para compliance total. Parceria fundacional: Dra. Juliana Pereira de Melo (OAB-GO 38.662), exclusividade B2B ate dez/2026.
...
| ----- | ----- | ----- | ----- |
| **Crisalida Z3 (em curso)** | **Abr-Jun 2026** | **WhatsApp Cloud API consolidada. Bitwarden ADR-009a. Tres Estados ADR-009b. Operacao piloto N=150 casos (2 CLTs + founders). OPEX R$18k/mes, BE 19-20 casos.** | **20+ casos/mes em 2 ciclos** |
| Fundacao | Q2-Q3 2026 | MCP Servers core (PostgreSQL, OCR). Tier 1+2 implementados. Router-Ethics como Tool MCP. | 150 usuarios piloto concluidos |
| Consolidacao | Q3-Q4 2026 | Tier 3+4 completos. Grimorio RAG como MCP Server. Checkpointer PostgreSQL. LangFuse integrado. Grimorio Acidentario v1 (B46/B91/B94). | 500 usuarios pagantes cumulativos |
