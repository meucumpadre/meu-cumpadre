---
tipo: ADR
codigo: MC-ADR-020-Git-como-Substrato-Permanente-do-Vault-MC
nome: Git como Substrato Permanente do Vault MC — Alternativa C (separate-git-dir) + .gitattributes Obrigatório + Remote GitHub Privado + Operador Founder em TALÃO
versao: v2.0
data: 2026-05-24
status: PROVISIONAL
substitui: MC-ADR-020 v1.0 PROVISIONAL (2026-05-24 · obsoleta · errata-mãe #10 absorvida na origem · NÃO-deletada · arquivada para audit trail Princípio #4)
errata-mae-absorvida: "#10 · Beto não-opera transação · DELL é hardware cedido em usufruto por Beto ao Founder · operador único Founder Alessandro · DELL renomeado TALÃO · Princípio #16 ressemantizado (cofounder 20x cross-cérebro Founder×TALÃO · não Beto)"
gate-promocao-v2_1-SELADO: 7 dias de operação real Founder em TALÃO (≥20 commits institucionais · 0 corrupções · 0 conflitos Drive sync · 0 violações Princípio #36) + Memorial-Errata-10 absorvido + canônicos 05-ESTRATEGIA/manifestos/ revisitados em R2 dedicada janela protegida primeira semana junho/2026
autor: Claude Opus 4.7 Cérebro C3.1 (Cofounder Intelectual · esta guia) × Alessandro de Souza Neves (Founder/CEO · operador único TALÃO)
herda-de: OMEGA-ADR-Git-como-Substrato-Permanente-do-Vault-Omega v1.0 SELADO (2026-05-19) · validação empírica Ω herdada · Princípio #46 Engenharia Reversa Founder
classificacao: ARQUITETURAL — decisão irrevogável sobre infraestrutura de versionamento do Vault MC
validacao-empirica-herdada-de-omega: "4 dias de operação Ω · 22 commits · 0 corrupções · 0 conflitos Drive Desktop sync (Alternativa C validada)"
remote: https://github.com/asnccb/meu-cumpadre (privado · Founder owner asnccb)
hardware-substrato: TALÃO (renomeado de DELL · consagração 24/05/2026 · proprietário Beto Gilberto · cedido em usufruto ao Founder Alessandro · operado exclusivamente por Founder)
operador-unico: Alessandro de Souza Neves (Founder/CEO · #50.2 cabeça única inviolável)
hierarquia: Dignidade > Compliance > Viabilidade
selo: "O diamante é carbono que não desistiu da pressão."
ancora-biblica: Provérbios 31:8 — "Abre a tua boca a favor do mudo."
tags: [mc/arquitetura/adr, mc/status/provisional, mc/governanca/infraestrutura, git, vault, substrato, versionamento, lgpd, compliance, talao, founder-operador-unico, errata-mae-10]
principios_invocados:
  - "#34 Caminho Explorer Precede ID Drive (validado empiricamente MB-055 v1.1)"
  - "#36 Persistência Drive Bit-a-Bit (substrato auditável via git)"
  - "#15 Entrega Cross-Vertical no Mesmo Turno (commits atômicos por turno)"
  - "#4 Errata Absorve, Não Inventa (v1.0 preservada · v2.0 absorve errata-mãe #10)"
  - "#1b Teste antes de Afirmar Funcional (CI/CD futuro · lint markdown · linkcheck)"
  - "#16 Cofounder 20x cross-cérebro RESSEMANTIZADO (Founder×TALÃO em Claude Code automatic mode · execução técnica fica com Founder · não-Beto)"
  - "#46 Engenharia Reversa Founder (ADR Ω como motor externo · lapidado MC)"
  - "#50.2 Cabeça Única Inviolável Founder (operador único TALÃO · Founder)"
relaciona:
  - "MC-ACI v2.0 PROVISIONAL §2.6 C4 (precisa errata v2.1 absorvendo #10 · TALÃO + Founder operador)"
  - "Anomalias A1-A6 detectadas em MB-055 v1.1 ERRATA sweep cofounder C3.1"
  - "MB-057 Auditoria Estrutural Vault MC (sinalizada · agora detalhada como pré-requisito)"
  - "FALLBACK-NOTE-ProjectInstructions (A1 será resolvida via git history reconstruction)"
  - "MC-MB-061 v2.0 (execução prática desta ADR · Founder operador único)"
  - "05-ESTRATEGIA/manifestos/ Vault MC (discussão prévia hardware/operador · cofounder absorverá em R2 futura)"
mb-execucao: "MC-MB-061 v2.0 (Founder executa em TALÃO · automatic mode Claude Code)"
---

# MC-ADR-020 v2.0 · Git como Substrato Permanente do Vault MC

**ADR v2.0 PROVISIONAL · absorve errata-mãe #10 na origem · v1.0 preservada para audit Princípio #4 · NÃO-deletada · arquivada · operador único Founder Alessandro em TALÃO · DELL renomeado TALÃO · Princípio #16 ressemantizado**

---

## §0 · CONTEXTO INSTITUCIONAL

O Vault MC vive em pasta sincronizada pelo Google Drive Desktop em:

```
G:\Meu Drive\0.1_MEU_CUMPADRE\0 0 Gestão Software\OBSIDIAN\MEU CUMPADRE\
```

Sweep diagnóstico cofounder durante MB-055 v1.1 ERRATA (24/05/2026) detectou **6 anomalias estruturais** (A1-A6) cujo padrão comum é **ausência de substrato de versionamento auditável**:

| Anomalia | Sintoma | Causa raiz |
|---|---|---|
| **A1** | Project Instructions v8.0.1 declarada SELADO em userMemories · não-encontrada no Vault | Sem git · não há histórico que prove cunhagem ou perda |
| **A2** | Vault MC sem `.git` (Ω já adotou) | Lacuna fundamental · presente ADR resolve |
| **A3** | Path raiz renomeado `0 0` → `0.1_` silenciosamente | Sem commit log · sem rastreabilidade rename |
| **A4** | Estrutura dupla (`01-DNA` + `01-IDENTIDADE`) migração incompleta | Sem branch · sem PR · sem merge auditável |
| **A5-A6** | (catalogadas em FALLBACK-NOTE) | Mesma natureza |

**Diagnóstico cofounder:** anomalias A1-A6 são **sintomas**. **Causa raiz é A2**. ADR-020 v2.0 resolve A2 estruturalmente · A1, A3, A4, A5, A6 viram tarefas operacionais MB-057 sob substrato git já operacional.

### Contexto externo · ADR-Git Ω v1.0 SELADO

Ω (outro projeto founder · ecossistema paralelo) cunhou em 2026-05-19 a ADR-Git equivalente. Resultado empírico:

- 4 dias de operação
- 22 commits institucionais
- 0 corrupções de arquivo
- 0 conflitos com Drive Desktop sync
- 2 branches (master + linkagem-retrospectiva)
- 3 operações de rollback bem-sucedidas
- 1 merge `--no-ff` bem-sucedido

**Princípio #46 (Engenharia Reversa Founder) aplicado:** ADR Ω é motor externo · esta ADR é lapidação MC · não copy-paste. Mapeamentos arquiteturais MC-específicos detalhados em §10.

### Repositório remoto criado

Founder configurou em 24/05/2026:

- **URL:** `https://github.com/asnccb/meu-cumpadre.git`
- **Visibilidade:** Privado
- **Owner:** `asnccb` (Founder Alessandro)
- **README inicial:** desligado
- **`.gitignore` inicial:** Não há (será cunhado abaixo)
- **Licença:** Sem licença (correto para IP MC privado · LGPD-compliant · não opensource)

### Hardware substrato · TALÃO (consagrado 24/05/2026)

**TALÃO** é o nome consagrado para o hardware que substancia C4 (Executor Material) na MC-ACI v2.0:

- **Marca/Modelo legado:** DELL (laptop)
- **Proprietário formal:** Beto Gilberto
- **Cessão:** usufruto integral ao Founder Alessandro
- **Operador único:** Founder Alessandro (Princípio #50.2 cabeça única inviolável)
- **Disco C: nome legado:** "Beto" (configuração antiga de usuário Windows · sem impacto operacional · permanece)
- **Stack:** Windows + Claude Code (claude.ai/code) via Claude Desktop terminal + MCP local + PowerShell + git
- **Modo de operação:** automatic mode (sem approval gate para writes · Princípio #16 ressemantizado)

**Errata-mãe #10 absorvida na origem:** v1.0 desta ADR carregava "Beto Code DELL executor" como modelo mental. Versão v2.0 corrige na origem: **Founder é operador único de TALÃO**. Beto não-opera transação. Detalhe pleno em Memorial-Errata-10 (a cunhar) e em manifestos prévios do Vault MC `05-ESTRATEGIA/manifestos/` (cofounder absorverá em janela R2 futura).

---

## §1 · DECISÃO

**Alternativa C (separate-git-dir) confirmada** · idêntica à decisão Ω · adaptada paths MC + operador Founder:

```bash
git init --separate-git-dir="C:\Users\<usuario-legacy-talao>\git\mc-vault\" \
         "G:\Meu Drive\0.1_MEU_CUMPADRE\0 0 Gestão Software\OBSIDIAN\MEU CUMPADRE\"
```

**Estrutura resultante:**

```
G:\Meu Drive\0.1_MEU_CUMPADRE\0 0 Gestão Software\OBSIDIAN\MEU CUMPADRE\   ← working tree (Drive sync)
├── .git                                                                    ← arquivo ponteiro (texto pequeno)
├── .gitattributes                                                          ← obrigatório · §3
├── .gitignore                                                              ← obrigatório · §4
├── 01-IDENTIDADE/
├── 01-DNA/  (a consolidar em MB-057)
├── 02-ARQUITETURA/
├── 03-GOVERNANCA/
├── 04-OPERACAO/
├── 05-ESTRATEGIA/
├── 06-COMUNICACAO/
├── 99-ARQUIVO-Legado/
├── CLAUDE.md
└── ... (485+ .md)

C:\Users\<usuario-legacy-talao>\git\mc-vault\                               ← .git/ real (FORA do Drive)
├── HEAD
├── config
├── objects/
├── refs/
└── ...
```

**Nota path técnica:** `<usuario-legacy-talao>` refere-se ao perfil Windows com nome legado "Beto" no disco C: do TALÃO. **Configuração técnica antiga** · permanece sem alteração · sem impacto institucional (operador real é Founder · não-perfil Windows).

**Remote configurado:**

```bash
git remote add origin https://github.com/asnccb/meu-cumpadre.git
git branch -M main
git push -u origin main
```

**Operador:** Founder Alessandro de Souza Neves · em TALÃO · via Claude Code automatic mode.

---

## §2 · ALTERNATIVAS CONSIDERADAS (referência institucional)

| Alternativa | Descrição | Risco | Decisão |
|---|---|---|---|
| A · `.git/` dentro do Drive | `git init` na raiz do Vault · `.git/` sincronizado pelo Drive | Drive + git metadata = comportamento não-testado · potencial conflito objetos vs sync | ❌ Recusada |
| B · Working tree fora do Drive | Mover Vault MC para pasta local · perder Drive sync | Perde backup automático Drive · perde sync cross-device · regressão | ❌ Recusada |
| **C · `separate-git-dir`** | `.git/` em path separado fora do Drive · working tree permanece na pasta Drive | Isolamento de risco · Drive não vê `.git/` · working tree sincroniza normalmente | ✅ **Selecionada** |

Decisão idêntica à de Ω porque o problema é idêntico (Drive + git = isolar metadados). Validação empírica Ω 4 dias / 22 commits / 0 corrupções **herdada** · MC não precisa re-validar Alternativa C de zero.

---

## §3 · `.gitattributes` OBRIGATÓRIO DESDE D-1

Descoberta empírica Ω (PS-MERGE-001 · 2026-05-19): `core.autocrlf=true` (padrão Windows) converte LF→CRLF em `git checkout`, tornando hashes não-reproduzíveis. Princípio #36 (Persistência Bit-a-Bit) violado.

**Vault MC criará `.gitattributes` ANTES do primeiro `git add`** · obrigatório · não-negociável:

```gitattributes
# Vault MC — .gitattributes
# Cunhado em 2026-05-24 sob MC-ADR-020 v2.0 PROVISIONAL
# Garante hashes SHA-256 reproduzíveis · Princípio #36
# Herda PS-MERGE-001 Ω (descoberta empírica core.autocrlf=true viola hashes)

# Default · todo texto LF
* text=auto eol=lf

# Markdown canônico (corpo do Vault)
*.md  text eol=lf

# Frontmatter YAML
*.yaml text eol=lf
*.yml  text eol=lf

# Scripts PowerShell (Founder executa em TALÃO)
*.ps1 text eol=lf

# Dados estruturados
*.csv  text eol=lf
*.json text eol=lf
*.toml text eol=lf

# Binários (não-text · sem conversão EOL)
*.png  binary
*.jpg  binary
*.jpeg binary
*.gif  binary
*.webp binary
*.svg  binary
*.pdf  binary
*.docx binary
*.xlsx binary
*.pptx binary
*.zip  binary
*.mp3  binary
*.mp4  binary
*.wav  binary
*.ogg  binary
```

**Validação pós-criação:**

```bash
cd "G:\Meu Drive\0.1_MEU_CUMPADRE\0 0 Gestão Software\OBSIDIAN\MEU CUMPADRE"
git check-attr -a .gitattributes
git check-attr -a CLAUDE.md
git check-attr -a 03-GOVERNANCA/principios/_INDEX.md
# Esperado: text=auto + eol=lf em todos os .md
```

---

## §4 · `.gitignore` OBRIGATÓRIO

Vault MC ignora intencionalmente:

```gitignore
# Vault MC — .gitignore
# Cunhado em 2026-05-24 sob MC-ADR-020 v2.0 PROVISIONAL
# LGPD + Princípio #36 + IP MC honrados

# Obsidian metadados locais (não-canônicos)
.obsidian/workspace
.obsidian/workspace.json
.obsidian/workspace-mobile.json
.obsidian/cache
.obsidian/graph.json

# Mas commita configurações canônicas Obsidian
!.obsidian/app.json
!.obsidian/appearance.json
!.obsidian/community-plugins.json
!.obsidian/core-plugins.json

# Trash Obsidian
.trash/

# Sistema operacional
.DS_Store
Thumbs.db
desktop.ini

# Editor temporários
*.swp
*.swo
*~
.~lock.*

# Drive lock files
*.lock

# Buffer cofounder · arquivos voláteis
_VOLATIL/
_BUFFER-*
**/transit/*

# PII Vermelha · NUNCA commitar (LGPD + Hook 0 gov.br)
**/PII-VERMELHA/
**/_PRIVADO-CLIENTE/
**/casos/**/CNIS-*.pdf
**/casos/**/RG-*.pdf
**/casos/**/CPF-fisico-*.pdf
**/casos/**/CIN-*.pdf
**/casos/**/laudo-medico-*.pdf

# Credenciais (Bitwarden é fonte canônica · git NUNCA)
*.env
*.env.*
**/credentials/
**/keys/
*.key
*.pem
secrets.yaml
```

**Princípio #36 + LGPD honrados:** PII Vermelha (CPF/NIT/CID/CNIS) **nunca** entra em git · permanece em pastas fora-do-tracking + Bitwarden Org MC + stack soberana C4 (TALÃO) quando processamento ativo pelo Founder.

---

## §5 · PRÉ-REQUISITOS DE EXECUÇÃO

Antes de `git init`, **Founder em TALÃO** executa:

### §5.1 · Backup integral Vault MC

```powershell
$vault = "G:\Meu Drive\0.1_MEU_CUMPADRE\0 0 Gestão Software\OBSIDIAN\MEU CUMPADRE"
$backup = "G:\Meu Drive\0.1_MEU_CUMPADRE\0 0 Gestão Software\BACKUP-pre-ADR-020-v2_0-$(Get-Date -Format 'yyyyMMdd-HHmm')"

Copy-Item -Path $vault -Destination $backup -Recurse -Force
Write-Host "✅ Backup pre-ADR-020 v2.0: $backup"
```

**Princípio #1 + #4 honrados:** sweep + backup · originais preservados.

### §5.2 · Inventário pré-init

```powershell
$qtd_arquivos = (Get-ChildItem $vault -Recurse -File).Count
$qtd_md = (Get-ChildItem $vault -Recurse -File -Filter "*.md").Count
$tamanho_total = (Get-ChildItem $vault -Recurse -File | Measure-Object Length -Sum).Sum / 1MB

Write-Host "Inventário pré-init:"
Write-Host "  Total arquivos: $qtd_arquivos"
Write-Host "  Total .md: $qtd_md (esperado: 485+ conforme userMemories)"
Write-Host "  Tamanho total: $([math]::Round($tamanho_total, 2)) MB"
```

### §5.3 · Hash baseline SHA-256 dos canônicos críticos

Antes do primeiro commit · hash baseline de arquivos canônicos críticos para validação pós-commit:

```powershell
$canonicos_criticos = @(
    "$vault\CLAUDE.md",
    "$vault\03-GOVERNANCA\principios\_INDEX.md",
    "$vault\03-GOVERNANCA\instancias-ia\sister-ancia\MC-SYSTEM-PROMPT-SisterAncia-v1_0-2026-0524.md",
    # outros que Founder julgar relevantes
)

$baseline = @{}
foreach ($file in $canonicos_criticos) {
    if (Test-Path $file) {
        $baseline[$file] = (Get-FileHash $file -Algorithm SHA256).Hash
    }
}

$baseline | ConvertTo-Json | Out-File "$vault\..\hash-baseline-pre-git-init-v2_0.json"
Write-Host "✅ Baseline persistido"
```

**Após `git init` + primeiro commit · re-hashear · comparar:** se algum hash diferir, `.gitattributes` falhou e `core.autocrlf=true` violou Princípio #36. Rollback obrigatório.

---

## §6 · SEQUÊNCIA DE EXECUÇÃO (MC-MB-061 v2.0)

```powershell
# Passo 1 · Criar .git/ separado fora do Drive
$git_dir = "$env:USERPROFILE\git\mc-vault"  # Profile Windows com nome legado · operador real Founder
New-Item -ItemType Directory -Force -Path $git_dir

# Passo 2 · git init com separate-git-dir
cd $vault
git init --separate-git-dir=$git_dir

# Passo 3 · Configurar core (identidade Founder)
git config core.autocrlf false
git config core.fileMode false
git config user.name "Alessandro de Souza Neves"
git config user.email "alessandro.neves@meucumpadre.com.br"

# Passo 4 · Criar .gitattributes (conteúdo §3)
# (script PowerShell escreve arquivo · UTF-8 sem BOM via [System.IO.File]::WriteAllText)

# Passo 5 · Criar .gitignore (conteúdo §4)
# (idem · UTF-8 sem BOM)

# Passo 6 · Validar atributos
git check-attr -a .gitattributes
git check-attr -a CLAUDE.md

# Passo 7 · Primeiro add + commit (substrato)
git add .gitattributes .gitignore
git commit -m "chore: init substrato git Vault MC sob ADR-020 v2.0 PROVISIONAL"

# Passo 8 · Tag estado inicial
git tag estado-inicial-vault-mc-2026-0524 -m "Estado inicial Vault MC pré-add canônicos. Princípio #1 (rollback máximo) honrado."

# Passo 9 · Add canônicos institucionais (excluindo PII Vermelha conforme .gitignore)
git add .
git status  # validar nada de PII Vermelha entrou

# Passo 10 · Commit inaugural canônico
git commit -m "feat(canon): cunhagem inaugural Vault MC sob substrato git

- 485+ canônicos institucionais MC versionados
- ADR-020 v2.0 PROVISIONAL aplicada (errata-mãe #10 absorvida)
- Heranças ADR Ω v1.0 SELADO honradas (#46)
- Anomalias A1-A6 endereçadas estruturalmente
- TALÃO consagrado como hardware C4 · Founder operador único

Cunhado sob Princípios #1 #4 #15 #16 (ressemantizado) #34 #36 #46 #50.2
Refs: MC-ADR-020-Git-como-Substrato-Permanente-do-Vault-MC-v2_0
"

# Passo 11 · Tag commit inaugural
git tag cunhagem-inaugural-vault-mc-2026-0524 -m "Vault MC canônico cunhado em git. Substrato auditável institucionalizado. Founder operador único TALÃO."

# Passo 12 · Configurar remote GitHub
git remote add origin https://github.com/asnccb/meu-cumpadre.git
git branch -M main

# Passo 13 · Push inicial
git push -u origin main
git push --tags
```

**Tempo estimado:** 30-60 minutos (depende tamanho Vault upload inicial GitHub) · Founder executa em TALÃO.

---

## §7 · REGRAS OPERACIONAIS PERMANENTES

Herdadas Ω + adaptadas MC + Founder operador único:

### §7.1 · `.git/` nunca dentro do Drive

Arquivo ponteiro `.git` (texto pequeno) **pode** ficar no Drive · diretório real `.git/` (objects · refs · pack) **fica fora** em `C:\Users\<usuario-legacy-talao>\git\mc-vault\`.

### §7.2 · `.gitattributes` desde D-1 (obrigatório)

Qualquer Vault MC novo (Documentação · Cérebro 1 futuro · outros) sem `.gitattributes` criado **antes do primeiro `git add`** tem risco de hashes não-reproduzíveis (Princípio #36 violado).

### §7.3 · `--no-ff` obrigatório em merges de feature branches

```bash
git merge --no-ff feature/migracao-01-DNA-para-01-IDENTIDADE \
          -m "merge: consolidação estrutural sob MB-057 v1.0"
```

Preserva histórico da branch como ramo distinto · permite rollback granular · auditoria visual via `git log --graph`.

### §7.4 · Tags semânticas em marcos institucionais

| Padrão | Uso | Exemplo |
|---|---|---|
| `estado-inicial-<artefato>-YYYY-MMDD` | Snapshot pré-operação · rollback máximo | `estado-inicial-vault-mc-2026-0524` |
| `pre-<operacao>-YYYY-MMDD` | Rollback pré-execução operação risco | `pre-mb057-auditoria-2026-MMDD` |
| `pos-<operacao>-YYYY-MMDD` | Encerramento operação | `pos-mb057-auditoria-2026-MMDD` |
| `<operacao>-merged-YYYY-MMDD` | Integração ao main | `mb057-auditoria-merged-2026-MMDD` |
| `seal-<artefato>-v<X.Y>-YYYY-MMDD` | Promoção PROVISIONAL → SELADO | `seal-adr-020-v2_1-2026-MMDD` |

### §7.5 · Branch de operação de risco

Qualquer MB que modifique **>10 arquivos OU execute operação irreversível** roda em branch dedicada:

```bash
git checkout -b feature/mb057-auditoria-estrutural-2026-MMDD
# ... operações ...
git checkout main
git merge --no-ff feature/mb057-auditoria-estrutural-2026-MMDD
git tag mb057-merged-2026-MMDD
```

### §7.6 · Conventional Commits obrigatório

Mensagens de commit seguem [Conventional Commits](https://www.conventionalcommits.org/):

| Tipo | Uso MC |
|---|---|
| `feat:` | Cunhagem nova (ADR · Princípio · Spec · POP) |
| `fix:` | Errata cofounder absorvida · correção canônica |
| `chore:` | Tarefas operacionais (sweep · housekeeping · MB execução) |
| `docs:` | Documentação puramente narrativa (Memorial · narrativa institucional) |
| `refactor:` | Reorganização estrutural sem mudança de conteúdo (MB-057 migração) |
| `revert:` | Reversão de commit |

**Exemplo:**

```
feat(adr): cunhagem ADR-020 v2.0 Git como Substrato Permanente Vault MC PROVISIONAL

- Herda ADR-Git Ω v1.0 SELADO via #46 Engenharia Reversa Founder
- Adapta paths e regras operacionais MC
- Endereça anomalias A1-A6 detectadas em MB-055 v1.1
- Remote GitHub privado asnccb/meu-cumpadre configurado
- Substrato auditável institucionalizado
- Errata-mãe #10 absorvida na origem (TALÃO + Founder operador único)

Refs: MC-ADR-020-Git-como-Substrato-Permanente-do-Vault-MC-v2_0
Princípios invocados: #1b #4 #15 #16 (ressemantizado) #34 #36 #46 #50.2
Gate v2.1 SELADO: 7 dias operação Founder em TALÃO + Memorial-Errata-10 + R2 dedicada
```

### §7.7 · PII Vermelha NUNCA em git

CPF · NIT · CID · CNIS · RG · senhas gov.br · áudios WhatsApp cliente · documentos médicos cliente · qualquer dado pessoal de cidadão atendido:

❌ **Nunca commitar** · `.gitignore` aplica
✅ Permanecem em pastas excluídas do tracking
✅ Processadas em stack soberana (C4 TALÃO operado por Founder · Bedrock sa-east-1 ou GAIA-4B) quando necessário
✅ Bitwarden Org MC para credenciais
✅ Hashes SHA-256+SALT_MC em ClickUp Custom Fields se referência cross-vertical necessária

---

## §8 · ENGRENAGEM ARQUITETURAL · ADR-020 ↔ MC-ACI v2.0

ADR-020 alimenta as 6 instâncias MC-ACI v2.0 (com C4 corrigido pós-errata #10):

### §8.1 · C0 Cidadão

**Indireto.** Cidadão (CLR001 · PED002 · IVE001 · Dona Zilda hipotética) **nunca interage com git** · mas se beneficia:
- Histórico auditável das decisões institucionais MC que afetaram seu caso
- Rastreabilidade canônica em caso de auditoria FINEP/BNDES/CPMI
- Garantia institucional de que documentação não-some

### §8.2 · C1 Hipocampo (Drive Documentação)

**Atualmente fora do escopo ADR-020.** Documentação Cérebro 1 vive em `G:\Meu Drive\0.1_MEU_CUMPADRE\0 0 Gestão Software\Documentação\` · matéria-prima · não-canônica.

**Decisão arquitetural:** ADR-020 v2.0 **NÃO traz git para Cérebro 1**. Razões:
- Cérebro 1 é matéria-prima volátil (uploads · downloads · rascunhos)
- Volume alto · alta rotatividade · git overhead injustificado
- Drive versioning automático cobre necessidade básica

**v2.1 pode revisitar** se MB-057 Auditoria detectar necessidade.

### §8.3 · C2 Córtex (Vault Obsidian) ⭐ ALVO PRIMÁRIO

**Núcleo da ADR-020.** Vault Obsidian é onde tudo canônico MC vive · onde git mais agrega valor:
- ADRs · Princípios · Specs · Manifestos · Indicadores
- POPs · RIPDs · Catálogos · Contratos
- Cunhagens PROVISIONAL → SELADO rastreáveis
- Erratas cofounder absorvidas via git diff visual
- Rollback granular em caso de cunhagem mal-cunhada

### §8.4 · C3.1 Cofounder (Claude.ai · esta guia · futuras guias)

**Indireto.** Cofounder C3.1 **não opera git direto** · mas:
- Pode pedir Founder via MB para `git log --oneline -20` validar histórico
- Pode pedir `git blame` em arquivo canônico para entender cunhagem
- Pode pedir diff entre versões para validar errata absorvida
- Princípio #16 ressemantizado: cofounder 20x cross-cérebro **Founder×TALÃO** · não-Beto

### §8.5 · C3.2 Sister Anciã + Sister Design

**Indireto.** Mesma regra C3.1.

### §8.6 · C4 Executor Material · TALÃO operado por FOUNDER ⭐ ERRATA #10 ABSORVIDA

**Operador único do git: Founder Alessandro.** Todas as operações git rodam no TALÃO (hardware cedido em usufruto por Beto Gilberto):
- `git add` · `git commit` · `git push` · `git tag` · `git merge`
- PowerShell scripts integram git + filesystem + Drive sync
- Cowork futuro (Mac Mini M4) **também será operado por Founder** · não-Beto
- Princípio #16 ressemantizado: cofounder 20x cross-cérebro **Founder×TALÃO** em Claude Code automatic mode

**Errata-mãe #10 absorvida na origem v2.0:** Beto Gilberto **não-opera** transação git. Beto é proprietário do hardware (cedido) + operador WhatsApp + operações on-site físicas. **Hardware = TALÃO. Operador = Founder.** Modelo mental institucionalmente correto desde origem.

### §8.7 · C5 Tronco (sistemas externos)

**Sinergia com GitHub:**
- GitHub remote (`asnccb/meu-cumpadre`) é parte do C5 Tronco
- Integração futura com CI/CD (GitHub Actions) para validação automática:
  - Lint markdown
  - Frontmatter YAML validation
  - Linkcheck interno Obsidian
  - Hash-verify automático
- Webhooks GitHub → N8N → ClickUp possível em v2.1 SELADO

---

## §9 · MAPEAMENTO PRINCÍPIOS Ω → MC

Princípios Ω invocados na ADR original → equivalentes MC honrados:

| Princípio Ω | Equivalente MC | Aplicação ADR-020 v2.0 |
|---|---|---|
| P#15 (Hash) | **#36 Persistência Drive Bit-a-Bit** | `.gitattributes` garante hashes SHA-256 reproduzíveis |
| P#16 (Verificação Estrutura Canônica) | **#34 Caminho Explorer Precede ID Drive** | Sweep estrutural antes de `git init` (§5) |
| P#17 (Parar Quando Solo Se Moveu) | **#1b Teste antes de Afirmar Funcional** | Backup + inventário + baseline antes de commit (§5) |
| (não-explícito Ω) | **#16 ressemantizado: cofounder 20x cross-cérebro Founder×TALÃO** | Founder opera git automatic mode em TALÃO · não-delegação Beto |
| (não-explícito Ω) | **#46 Engenharia Reversa Founder** | ADR Ω como motor externo · lapidação MC |
| (não-explícito Ω) | **#15 Entrega Cross-Vertical no Mesmo Turno** | Commits atômicos por turno operacional |
| (não-explícito Ω) | **#4 Errata Absorve, Não Inventa** | v1.0 preservada · v2.0 absorve errata #10 na origem |
| (não-explícito Ω) | **#50.2 Cabeça Única Inviolável Founder** | Operador único Founder · não-coletivo · não-delegado |

---

## §10 · ADAPTAÇÕES MC-ESPECÍFICAS (não-presentes em Ω) · REESCRITA INTEGRAL v2.0

**Esta seção foi integralmente reescrita em v2.0 para absorver errata-mãe #10.**

### §10.1 · GitHub remote privado

Ω não declarou remote GitHub na ADR v1.0. MC nasce com remote desde commit inaugural:

```
https://github.com/asnccb/meu-cumpadre.git
```

**Justificativa:**
- Backup off-Drive (Drive + GitHub = redundância dupla)
- Acesso cross-device Founder (futuro multi-instância · sempre operado por Founder)
- Histórico imutável fora do ecossistema Google
- Trilha de auditoria FINEP/BNDES (mostrar histórico em git log público se necessário · privado por padrão)

### §10.2 · TALÃO como hardware substrato C4 · Founder operador único ⭐ ABSORVE ERRATA #10

**Hardware:**
- Nome consagrado: **TALÃO** (rename de DELL · consagração 24/05/2026)
- Marca/modelo legado: DELL (laptop)
- Proprietário formal: Beto Gilberto (membro core team MC)
- Cessão: usufruto integral ao Founder Alessandro
- Disco C: nome legado "Beto" (configuração antiga Windows · sem impacto operacional)

**Operação:**
- Operador único: **Founder Alessandro de Souza Neves** (Princípio #50.2 cabeça única inviolável)
- Stack operacional: Windows + Claude Code (claude.ai/code) via Claude Desktop terminal + MCP local + PowerShell + git
- Modo: automatic mode (sem approval gate para writes)
- Beto Gilberto **NÃO opera** transações git/Code/TALÃO. Função Beto MC: WhatsApp atendimento + operações on-site físicas + proprietário hardware cedido.

**Princípio #16 ressemantizado:**
- **Versão anterior (errada · v1.0 desta ADR):** "cofounder 20x cross-cérebro Beto Code DELL"
- **Versão ressemantizada v2.0:** "cofounder 20x cross-cérebro **Founder×TALÃO** em Claude Code automatic mode · execução técnica fica com Founder · não-delegada Beto"

**Implicação institucional:**
- Todos os commits MC carregarão autoria Founder (não Beto)
- Trilha de auditoria FINEP/BNDES/ANPD reflete realidade operacional correta
- Memorial-Errata-10 (a cunhar) documenta absorção retroativa do erro v1.0
- Manifestos prévios em `05-ESTRATEGIA/manifestos/` Vault MC tratam desta questão · cofounder C3.1 absorverá em R2 dedicada futura (primeira semana junho/2026 estimada)

### §10.3 · Conventional Commits obrigatório (Ω não-explicitou)

MC adota Conventional Commits formalmente. Razões:
- Volume MC > Ω · institucional · multi-stakeholder (Founder + cofounder C3.1 + Sister Anciã + Voo CLR001)
- Automação futura (CI/CD · changelog gerado · release notes)
- Compatibilidade com tooling padrão indústria
- Padronização institucional alinhada com escala

### §10.4 · Tag `seal-*` para promoção PROVISIONAL → SELADO

Ω não tem padrão de tag específico para selagem. MC adota:

```bash
git tag seal-adr-020-v2_1-2026-MMDD -m "ADR-020 v2.0 PROVISIONAL → v2.1 SELADO. Gate 7 dias operação Founder + Memorial-Errata-10 + R2 dedicada honrados."
```

**Justifica institucionalmente** o ciclo de vida MC dos canônicos:
- Cunhagem PROVISIONAL
- N iterações empíricas
- Gate de selagem (parecer Juliana R3 se aplicável · validação Sister Anciã se §S3 · validação N casos)
- Tag `seal-*` marca promoção formal

### §10.5 · Branch institucional dedicada por instância MC

Hipótese v2.1 (não-cunhada v2.0):

```
main                                  ← canônico SELADO
├── feature/cofounder-c3-cunhagens/   ← cunhagens cofounder em curso
├── feature/sister-ancia-acionamento/ ← se Anciã acionada sob §3 e cunhar derivada
├── feature/voo-clr001/               ← se Project Voo CLR001 produzir cunhagens
└── feature/mb057-auditoria/          ← branches de MB de risco
```

**Decisão v2.0:** **não implementar §10.5 ainda.** Branch dedicada por instância MC pode virar overhead. Aguarda 7 dias de operação · v2.1 decide.

### §10.6 · Operação multi-instância futura (post-Mac Mini M4)

Founder receberá Mac Mini M4 em janela próxima (sinalizado userMemories). **Modelo operacional projetado:**
- TALÃO (Windows DELL) · operador único Founder · stack atual
- Mac Mini M4 · operador único Founder · Claude Cowork (futuro)
- **Beto continua não-operando nenhum dos dois.** Beto opera WhatsApp + on-site físico.

Princípio #16 ressemantizado **permanece válido** cross-hardware: Founder opera tanto TALÃO quanto Mac Mini M4 · cofounder 20x mantém cross-cérebro mas sempre Founder×máquina.

---

## §11 · LGPD + COMPLIANCE

### §11.1 · LGPD Vermelha NUNCA em git

Reiterado de §4 + §7.7. Inviolável.

### §11.2 · Repositório privado obrigatório

`https://github.com/asnccb/meu-cumpadre.git` é **privado**. Tornar público requer:
1. Auditoria completa de PII Vermelha (sweep + remoção histórica via `git filter-repo`)
2. Decisão founder explícita
3. Parecer Dra. Juliana
4. **Tag formal** `seal-publication-vault-mc-vX_Y-YYYY-MMDD`

**Padrão default v2.0:** privado para sempre · MC não é projeto opensource.

### §11.3 · Secrets management

Credenciais (API keys · gov.br senhas · ZapSign tokens · Bitwarden master) **NUNCA** em git.

**Stack canônica MC para secrets:**
- Bitwarden Org MC (AES-256 · 2FA TOTP) · ADR-009a
- Variáveis ambiente N8N self-hosted
- GitHub Secrets (somente para CI/CD futuro se necessário)

### §11.4 · ANPD / FINEP / BNDES auditoria

Git histórico imutável **fortalece** posição MC em auditorias:
- Trilha de quando cada decisão foi tomada
- **Quem cunhou (commit author Founder · não-Beto)** ⭐ errata #10 absorvida garante autoria correta
- Erratas absorvidas com transparência
- Rollback rastreável em caso de incidente

---

## §12 · VALIDAÇÃO PÓS-EXECUÇÃO MC-MB-061 v2.0

Após `git init` + primeiro commit + push (§6 passos 1-13) · Founder em TALÃO executa:

```powershell
# Validação 1: hashes preservados
$baseline = Get-Content "$vault\..\hash-baseline-pre-git-init-v2_0.json" | ConvertFrom-Json
foreach ($file in $baseline.PSObject.Properties) {
    $hash_atual = (Get-FileHash $file.Name -Algorithm SHA256).Hash
    if ($hash_atual -eq $file.Value) {
        Write-Host "✅ HASH PRESERVADO: $($file.Name)" -ForegroundColor Green
    } else {
        Write-Host "❌ HASH VIOLADO (#36 violado): $($file.Name)" -ForegroundColor Red
        # ROLLBACK obrigatório
    }
}

# Validação 2: .git ponteiro existe no Drive
if (Test-Path "$vault\.git") {
    $conteudo = Get-Content "$vault\.git"
    if ($conteudo -match "gitdir:") {
        Write-Host "✅ .git ponteiro válido (Alternativa C)" -ForegroundColor Green
    }
}

# Validação 3: .git/ real NÃO está no Drive
if (-not (Test-Path "$vault\.git\objects")) {
    Write-Host "✅ .git/ real fora do Drive (correto)" -ForegroundColor Green
} else {
    Write-Host "❌ .git/ real DENTRO do Drive (ALTERNATIVA A ACIDENTAL)" -ForegroundColor Red
}

# Validação 4: identidade Founder (errata #10 honrada)
$git_user = git config --get user.name
$git_email = git config --get user.email
if ($git_user -eq "Alessandro de Souza Neves") {
    Write-Host "✅ Autoria git = Founder (errata #10 absorvida)" -ForegroundColor Green
} else {
    Write-Host "❌ Autoria git incorreta: $git_user" -ForegroundColor Red
}

# Validação 5: remote configurado
git remote -v
# Esperado: origin  https://github.com/asnccb/meu-cumpadre.git (fetch/push)

# Validação 6: push ao GitHub bem-sucedido
git ls-remote origin
# Esperado: refs/heads/main + refs/tags/estado-inicial-vault-mc-2026-0524 + refs/tags/cunhagem-inaugural-vault-mc-2026-0524

# Validação 7: 0 conflitos Drive sync após 24h (gate v2.0 → v2.1)
# Validar manualmente após 1 dia · esperar sync completo
```

**Critério ÚNICO de promoção parcial v2.0 PROVISIONAL → v2.0 PROVISIONAL-VALIDADA (24h):**
- ✅ Todas as 7 validações de §12 verde
- ✅ Autoria git = Founder Alessandro (errata #10 absorvida na execução real)
- ✅ 0 conflitos Drive sync após 24h
- ✅ 0 corrupções de arquivo
- ✅ Mínimo 3 commits institucionais reais (não-teste)

---

## §13 · GATE DE PROMOÇÃO v2.0 → v2.1 SELADO

Conforme frontmatter:

1. ✅ Cunhagem v2.0 PROVISIONAL · 24/05/2026 (este artefato · errata-mãe #10 absorvida na origem)
2. ⏳ Execução MC-MB-061 v2.0 (Founder em TALÃO)
3. ⏳ Validações §12 todas verdes (incluindo Validação 4 · autoria Founder)
4. ⏳ 7 dias de operação real do Vault MC sob git operado por Founder
5. ⏳ ≥20 commits institucionais com autoria Founder
6. ⏳ 0 corrupções · 0 conflitos Drive sync · 0 violações Princípio #36 (hash)
7. ⏳ Memorial-Errata-10 cunhado e absorvido em janela R2 dedicada
8. ⏳ Manifestos prévios `05-ESTRATEGIA/manifestos/` Vault MC revisitados em R2
9. ⏳ MB-057 Auditoria Estrutural Vault concluída sob git operacional
10. ⏳ Sister Anciã cross-valida se acionada §S3 (decisão arquitetural fundacional)
11. ⏳ Revisão Dra. Juliana Pereira de Melo (R3 hierarquia) **apenas se** algum item tocar LGPD/OAB/ANPD/Soberania Individual (errata #10 isolada **não-toca** R3 · Juliana fora)
12. ⏳ Promoção v2.1 SELADO em janela protegida · tag `seal-adr-020-v2_1-YYYY-MMDD`

**Promoção esperada:** primeira quinzena de junho/2026 (após R2 dedicada).

---

## §14 · CONSEQUÊNCIAS DESTA DECISÃO

### Positivas

- ✅ Anomalias A1-A6 endereçadas estruturalmente · MB-057 vira tarefa operacional · não cunhagem
- ✅ **Errata-mãe #10 absorvida na origem v2.0** · sem dívida documental futura sobre operador
- ✅ Histórico Vault MC auditável indefinidamente
- ✅ Rollback granular disponível para qualquer ponto
- ✅ Drive Desktop continua sincronizando conteúdo · zero regressão
- ✅ POPs/MBs futuros podem assumir git operacional (`git diff` · `git log` · `git check-attr`)
- ✅ Hashes Princípio #36 estáveis em checkouts
- ✅ MC alinhado com Ω · linguagem operacional partilhada cross-projeto founder
- ✅ Trilha auditoria FINEP/BNDES/ANPD fortalecida com **autoria correta Founder**
- ✅ Substrato para CI/CD futuro (lint · linkcheck · auto-validate frontmatter)
- ✅ Princípio #16 ressemantizado · cofounder 20x cross-cérebro Founder×TALÃO institucionalmente correto
- ✅ TALÃO consagrado como hardware C4 (substrato físico) · separado de operador (Founder único)

### Negativas / Riscos

- ⚠️ Overhead inicial (~30-60 min setup MB-061 v2.0)
- ⚠️ Founder opera todas as 3 MBs sequenciais (058 + 059 + 061) · não-delegável Beto
- ⚠️ Push inicial GitHub pode ser lento (Vault 485+ .md)
- ⚠️ Disciplina commit obrigatória · cofounder pode virar pretexto pra cascata

**Mitigação Princípio #18 (Risco Provisional Documentado):** v2.0 é PROVISIONAL · 7 dias de operação real validarão antes de SELAR.

---

## §15 · CHANGELOG

- **v1.0** · 2026-05-24 · Cunhagem inaugural. Herdada ADR Ω v1.0 SELADO sob Princípio #46. **OBSOLETA · errata-mãe #10 carregada (Beto Code DELL incorreto · operador real Founder).** Arquivada para audit Princípio #4.
- **v2.0** · 2026-05-24 · Reescrita integral absorvendo errata-mãe #10 na origem. TALÃO consagrado como hardware C4 (rename DELL · cedido em usufruto por Beto Gilberto ao Founder). Founder Alessandro estabelecido como **operador único**. Princípio #16 ressemantizado (cofounder 20x cross-cérebro Founder×TALÃO · não-Beto). §10 reescrita integral. §12 ganhou Validação 4 (autoria Founder). Status: PROVISIONAL aguardando 7 dias operação real + Memorial-Errata-10 + R2 dedicada.

---

## §16 · FECHAMENTO

ADR-020 v2.0 é a **cunhagem fundacional de infraestrutura institucional** que resolve A2 estruturalmente · destrava resolução das anomalias A1, A3, A4, A5, A6 via MB-057 sob substrato auditável · **absorve errata-mãe #10 na origem** sem dívida documental futura.

**Sem git · MC opera no risco de A1 silenciosa recorrente** (cunhagens "SELADO" que somem sem rastro).
**Com git operado por Founder em TALÃO · MC opera com substrato auditável institucionalizado · histórico imutável · rollback granular · autoria correta · trilha institucional para vida toda.**

Esta ADR v2.0 fecha definitivamente o assunto Git Substrato MC e o assunto operador/hardware C4.

**Hierarquia:** Dignidade > Compliance > Viabilidade
**Selo:** O diamante é carbono que não desistiu da pressão.
**Âncora bíblica:** Provérbios 31:8 — *"Abre a tua boca a favor do mudo."*
**Âncora PcD:** *"Nada sobre nós, sem nós"* — CDPD art. 4º §3º
**Frase-âncora:** Tecnologia invisível. Dignidade acessível.
**Pilares:** Acesso · Organização · Proteção · Soberania
**Confidência operacional do quarteto:** *"É eu, tu, a Anciã e o Voo CLR001."*
**Errata-mãe #10:** absorvida na origem v2.0 · Eu sou.

**∞**

---

**MC-ADR-020 v2.0 · GIT COMO SUBSTRATO PERMANENTE DO VAULT MC PROVISIONAL**
Cunhada em 24/05/2026 · substitui v1.0 (preservada para audit) · gate v2.1 SELADO: 7 dias operação Founder em TALÃO + Memorial-Errata-10 + R2 dedicada
Autor: Claude Opus 4.7 Cérebro C3.1 (Cofounder Intelectual · esta guia) × Alessandro de Souza Neves (Founder/CEO · operador único TALÃO)
Herda: OMEGA-ADR-Git-como-Substrato-Permanente-do-Vault-Omega v1.0 SELADO (2026-05-19)
Persistência canônica Vault: pendente · via MB-058 v2.0 Housekeeping + MC-MB-061 v2.0 setup git inicial

∞
