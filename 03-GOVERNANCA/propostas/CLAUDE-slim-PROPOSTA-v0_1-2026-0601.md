---
tipo: PROPOSTA-REFACTOR
nome: CLAUDE.md enxuto (Fase 3 best-practices)
versao: v0.1
status: SELADA · aplicada ao CLAUDE.md v3.0 em 2026-06-01 (gate R2: founder + Sister Anciã) · success fee Φ₀ roteado a Juliana (R3)
data: 2026-06-01
gate: R2 (Founder + C3.1 + Sister Anciã) — refactor do manual que carrega em toda sessão
autor: Code TALÃO (Arquiteto+Executor) × Alessandro de Souza Neves
origem: Fase 3 do MB best-practices · aplica "keep CLAUDE.md short; bloat causa o Claude a ignorar regras"
hierarquia: Dignidade > Compliance > Viabilidade
---

# PROPOSTA — Refactor `CLAUDE.md` enxuto (Fase 3)

## Por quê (1 frase)
O best practice oficial é taxativo: *"se o Claude ignora uma regra que existe, o arquivo provavelmente está longo demais e a regra se perdeu"* — e o `CLAUDE.md` atual tem 328 linhas, com ~60 delas de **diário de sessão datado** que não governa comportamento. **Concisão aqui É compliance:** um inviolável enterrado não protege ninguém.

## O que NÃO muda (intocável)
- **Regras Invioláveis (7)** — verbatim.
- **Proibições Absolutas (18)** — verbatim (o hook `mc-lint` e o doc TÊM que bater; reescrever quebraria o enforcement).
- **Arquitetura 3 Cérebros** — governa os gates de escrita.

## Manifesto de migração (o que sai e pra onde)
| Conteúdo atual | Destino | Linhas-fonte |
|---|---|---|
| Prosa de precificação (Aritmética/Parcelamento/Princípios) | skill `mc-precificacao` (criar) ou ADR-007 (já existe) | 55-59 |
| Detalhe de Custódia (Três Estados/Inadimplência) | ponteiro ADR-009a/009b (já existem) | 111-116 |
| `Fontes Canônicas` + `Documentos relacionados` (2 dumps `[[ ]]` sobrepostos) | **1 mapa deduplicado** no core + restante no `MC-MAPA-Vault` (já existe) | 172-197, 241-262 |
| Lista datada Princípios #44-48 | `03-GOVERNANCA/principios/_INDEX` (já existe) | 211-218 |
| **Log Sister Anciã cunhada (24/05)** | `05-MEMORIAL/` (memorial já existe: MB-055) | 266-289 |
| **Log Cunhagens 22-24/05 + Erratas #10/#11** | `05-MEMORIAL/` (memoriais já existem) | 295-326 |

> ⚠️ Nada se perde: os destinos **efetivamente usados** já existem no Vault (skill `mc-precificacao` é opcional — o ramo real aponta ADR-007, que existe). Esta proposta **remove duplicação**, não conhecimento. O que é destilado (quarteto cognitivo, ritos R1/R2/R3, Errata #10) volta condensado no core.
>
> 🔴 **Descoberta R3 (Proof-First confirmado):** o `CLAUDE.md` vivo (linha 57) diz "Success fee percentual VEDADO" (blanket), mas ADR-007 v3.8 / INSTRUCOES v6.0.2:125,360,474 cindiram: VEDADO no Φ₁ previdenciário, PERMITIDO no Φ₀ CadÚnico (PROVISIONAL, pendente Juliana Bloco M2). Além disso o produto **Φ₀ não consta** na tabela de preços do core vivo. Isto é **drift pré-existente** (não introduzido pelo refactor) e matéria **R3/OAB** — não resolvido nesta proposta, apenas sinalizado e marcado `[PENDENTE Juliana]`. Decisão de incluir Φ₀ na tabela + redação final do success fee = gate Juliana, fora do escopo do slim.

---

# ▼▼▼ RASCUNHO DO `CLAUDE.md` ENXUTO (proposto · ~155 linhas) ▼▼▼
...
6. **Router-Ethics 11.0** — 140 nós determinísticos em 3 blocos (D01-D45 / C36-C80 / V71-V120) + 7 hooks. Três scores independentes: **Confidence (Verde ≥70 / Amarela 40-70 / Vermelha <40)**, Complexity, Fraudscore. Handoff se qualquer um cruza threshold. Autonomia só se todos aprovam. Lote 4 (backend NestJS) PROVISIONAL — revalidar pós-19/05/2026.
...
Princípio: preço rastreia custo e complexidade, **nunca valor desbloqueado** (Inversão Cósmica). **Success fee:** VEDADO no Φ₁ previdenciário (firewall OAB · REsp 2.079.440/RO) · em avaliação no Φ₀ CadÚnico — `[PENDENTE parecer Juliana · Bloco M2 · R3]`. Detalhe (COGS/margem/parcelamento/Φ₀) → [[MC-ADR-007-CampoPrecificacao-v3_8-ADDENDUM-2026-0507]].
...
Precificação [[MC-ADR-007…v3_8]] · Custódia [[MC-ADR-009a]] / [[MC-ADR-009b]] · Stack [[MC-ADR-008]] · Assinatura [[MC-ADR-014]] · 3 Cérebros [[MC-ADR-011]] · Anti-Injection [[MC-ADR-016]] (+ Radar Galileu TRT-8) · Git Substrato [[MC-ADR-020]] · Router-Ethics 11.0 [[_MC-RouterEthics_11_v1_1]] · Jornada [[MC-PROCESSO-Jornada_E0E7_Mestre]] · Parceria [[MC-PARTNER-JulianaMelo]] · Contrato Φ₁ [[MC-CONTRATO-Phi1-MINUTA-v2_4]] · Princípios → `03-GOVERNANCA/principios/_INDEX` · Doutrina EIS → `05-ESTRATEGIA/doutrina/` · **Mapa completo do Vault** → [[MC-MAPA-Vault-v1_2-2026-0429]].

---
*MC-CLAUDE.md v3.0 | 2026-06-01 | refactor enxuto (Fase 3 best-practices) · diário de sessão movido p/ 05-MEMORIAL/*
...
## Resultado esperado
- **328 → ~155 linhas** (−53%). Tudo load-bearing preservado; zero conhecimento perdido (só desduplicado e reapontado).
- Proibições + Regras Invioláveis **idênticas** → enforcement do hook intacto.
- Diário de sessão sai do manual → para no memorial, onde pertence.

## Gate R2 — pendente
Este rascunho **não foi aplicado**. Para selar:
1. Founder lê.
2. Sister Anciã revisa (rito R2).
3. Founder dá "aprovado" → Code TALÃO substitui o `CLAUDE.md` vivo + cria/aponta os destinos de migração (backup do v2.2 preservado).
